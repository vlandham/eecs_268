...
p((. %Stack(const Stack&amp; aStack);%

becomes

p((. %Stack(const Stack&lt;T&gt;&amp; aStack);%
...
p((. %void Stack::push(const StackItemType&amp; newItem) throw(StackException)%

Now as a method template it will look like:

p((. %template &lt;class T&gt;%

p((. %void Stack&lt;T&gt;::(const T&amp; newItem) throw(StackException)%
...
p((. %int aFunction  (float a, char b, char c)%

p((. %{ ... }%

Then we could create a function pointer like:

p((. %int (*pt2Function)(float, char, char) = NULL;%

and assign it to the aFunction class like:

p((. %pt2Function = &DoMore;%
...
p((. %static SolverClass* thisPointer%
...
p((. %static void Wrapper_To_Call_Solve(char&amp; val); //wrapper%

p((. %void solve(char&amp; val); //actual method%

Now our implementation of the wrapper method will use this thisPointer to call the real solve method.

p((. %void SolverClass::Wrapper_To_Call_Solve(char&amp; val)%

p((. %{%

p(((. %thisPointer->solve(val); // calls the actual non-static solve with the input it got.%
