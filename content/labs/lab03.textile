...
h2. Using putback to replace content on an input stream

<pre>
  <code>
    string command, event_name;
  	int position;

  	while (fin >> command)
  	{
  		if (command == "new-contestant")
  		{
  			fin >> event_name;
  			...
  			// Replace character
  			fin.putback(check_position);

  			// Read in position
  			if (isdigit(check_position)) fin >> position;
  			else position = 0;
